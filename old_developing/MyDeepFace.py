...
def detect_faces(img_path, target_size = (224, 224), grayscale = False,
	...
	#Loads image. Image might be path, base64 or numpy array. Convert it to numpy whatever it is.
	img = functions.load_image(img_path)
    
	#print("Image shape: ", img.shape, "\nImage region: ", img_region)

	# The detector is stored in a global variable in FaceDetector object.
	# This call should be completed very fast because it will return found in memory
	# and it will not build face detector model in each call (consider for loops)
	face_detector = FaceDetector.build_model(detector_backend)
	detections    = FaceDetector.detect_faces(face_detector, detector_backend, img, align)
	...
	# Raises an error if return type is not 'faces', 'regions' or 'both'. Otherwise, initializes lists.
	if return_type == "faces" or return_type == "regions" or return_type == "both":
		faces = []
		rois  = []
	else:
