...
class FaceAlignmentDetector(BaseFaceDetector):
    def __init__(self, 
        ...
        if fd_type.lower() == "s3fd":
            ...
            raise ValueError(f"Unknown face detector {fd_type}.")
    ...
    def detect_face(self, image, with_landmarks=True):
        """
        Returns: 
            bbox_list: bboxes in [x0, y0, x1, y1] ordering (x is the vertical
            axis, y the height).

            landmarks_list: landmark points having shape (68, 2) with ordering
            [[x0, y0], [x1, y1], ..., [x67, y67].
        """
    ...
    def preprocess_s3fd_bbox(bbox_list):
        ...
        return [np.array([bbox[1], bbox[0], bbox[3], bbox[2], bbox[4]]) \
                for bbox in bbox_list]
