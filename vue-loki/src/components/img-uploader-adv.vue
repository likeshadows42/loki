...
<template>
  <h2>Multiple images uploader</h2>
  <div class="container"
    ...
    <div v-show="Imgs.length == 0" class="beforeUpload">
      <input type="file"
        ...
        multiple
      />
      <img class="imgIcon" src="../assets/uploader-folder-icon.svg">
      <p class="mainMessage">
    ...
    <div class="imgsPreview" v-show="Imgs.length > 0">
      ...
      <div class="imageHolder" v-for="(img, i) in Imgs" :key="i">
        ...
        <span class="delete" style="color: white" @click="deleteImg(--i)">
          <img src="../assets/img-uploader-bin-icon.svg" style="color: white">
        </span>
        <div class="plus" @click="append" v-if="++i == Imgs.length">+</div>
      </div>
    </div>

    <!-- <button v-show="Imgs.length > 0" type="button" class="clearButton" @click="reset">
      {{ clearAll ? clearAll : "clear All" }}
    </button> -->

    <button v-show="Imgs.length > 0" @click.prevent="uploadFiles(files)" class="upload-button">Upload</button>
