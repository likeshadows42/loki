...
export default {
  ...
  methods: {
    ...
    async getGroupMembers(person_id) {
      const params = {}
      this.group_obj = await this.axiosPost(`/api/people/get_faces?person_id=${person_id}`, params)
      this.group_num = Object.keys(this.group_obj).length
    },

    async removeImg(item_id, person_id) {
      const params = {}
      await this.axiosPost(`/api/facerep/unjoin?face_id=${item_id}`, params)
      this.getGroupMembers(person_id)
    },

    async updatePerson(person_id) {
      const params = {}
      await this.axiosPost(`/api/people/set_name?person_id=${person_id}&person_name=${this.person_new_name}`, params)
      await this.axiosPost(`/api/people/set_note?person_id=${person_id}&new_note=${this.person_new_note}`, params)
